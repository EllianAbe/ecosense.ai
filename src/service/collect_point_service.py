...
class CollectPointService:
    ...
    def create_collect_point(self, description: str, cep: str, street: str, number: str, city: str, state: str, contact_name: str, phone: str, email: str):
        ...
        try:
            latitude, longitude = geolocation_service.get_coordinates(
                f"{street}, {number}, {city}, {state}, {cep}")

            new_collect_point = CollectPoint(description=description, cep=cep, street=street, number=number,
                                             city=city, state=state, contact_name=contact_name, phone=phone,
    ...
    def read_collect_points(self):
        try:
            collect_points = self.db.query(
                CollectPoint).order_by(CollectPoint.id).all()
    ...
    def get_collect_point(self, collect_point_id: int):
        try:
            collect_point = self.db.query(CollectPoint).filter(
                CollectPoint.id == collect_point_id).first()
    ...
    def update_collect_point(self, collect_point_id: int, description: str, cep: str, street: str, number: str, city: str, state: str, contact_name: str, phone: str, email: str):
        ...
        try:
            ...
            if collect_point:
                ...
                collect_point.latitude = latitude
                collect_point.longitude = longitude
    ...
    def delete_collect_point(self, collect_point_id: int):
        ...
        try:
            collect_point_to_delete = self.db.query(CollectPoint).filter(
                CollectPoint.id == collect_point_id).first()
